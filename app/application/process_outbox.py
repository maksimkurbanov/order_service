...
from app.domain.models import OutboxStatusEnum, EventTypeEnum, OrderStatusEnum
from app.infrastructure.http_clients import NotificationsServiceClient
            ...
            for event in events:
                for attempt in range(1, self._max_retries + 1):
                    async with uow.session.begin_nested():
                        try:
                            if event.event_type == EventTypeEnum.PAID:
                                async with self._kafka_producer as kp:
                                    log.debug("Sending event to Kafka: %s", event)
                            ...
                            await self._notifications_client.send_notification(
                                ...
                                idempotency_key=uuid5(event.order_id, event.event_type),
                            )
                            ...
                            log.warning(
                                "Attempt %s/%s failed for event: %s %s: %s",
                                attempt,
                                self._max_retries,
                                event.order_id,
                                event.event_type,
